...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ 
    ...
    return (
      <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-200">
        ...
        <div onClick={() => onOpenHub('PROFILE')} className="flex flex-col items-center gap-6 bg-black/40 p-8 rounded-3xl border border-white/10 backdrop-blur-md cursor-pointer group hover:bg-black/60 transition-all active:scale-[0.98] relative overflow-hidden">
           <div className="absolute inset-0 bg-gradient-to-tr from-yellow-500/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200"></div>
           <div className="relative">
              <div className="absolute inset-[-20px] bg-yellow-500/10 blur-[40px] rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300"></div>
              
              <div className="absolute -top-1 -right-1 z-30 bg-black border-2 border-yellow-500 text-yellow-500 px-2 py-0.5 rounded-lg shadow-2xl flex items-center gap-1 animate-in zoom-in duration-300 select-none">
                 <span className="text-[10px] font-black tracking-tighter">LVL {currentLevel}</span>
              </div>

              <div className="relative w-28 h-28 rounded-full bg-black/40 border border-yellow-500/30 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300 overflow-hidden"><VisualEmote trigger={playerAvatar} remoteEmotes={remoteEmotes} size="xl" /></div>
           </div>
           <div className="flex flex-col items-center gap-1.5">
              ...
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/5 group-hover:bg-yellow-500/10 group-hover:border-yellow-500/30 transition-all duration-200"><span className="text-[7px] font-black text-yellow-500/60 uppercase tracking-[0.4em]">Update Credentials</span></div>
           </div>
        </div>
        <div onClick={() => onOpenHub('LEVEL_REWARDS')} className="bg-black/40 backdrop-blur-md border border-white/5 p-5 rounded-3xl space-y-2 cursor-pointer hover:bg-white/[0.02] transition-colors duration-200 group">
          <div className="flex justify-between items-end"><span className="text-[10px] font-black text-white/40 uppercase tracking-widest group-hover:text-white transition-colors duration-200">Level {currentLevel}</span><span className="text-[10px] font-black text-yellow-500 uppercase tracking-widest">{nextLevelXp - (profile?.xp || 0)} XP TO RANK UP</span></div>
          <div className="h-2 w-full bg-black/40 rounded-full overflow-hidden border border-white/5"><div className="h-full bg-yellow-500 transition-all duration-700" style={{ width: `${progress}%` }} /></div>
        </div>
      </div>
    );
  };
  ...
  const renderCustomizeTab = () => {
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
        <SectionLabel 
          rightElement={
            <div className="flex flex-col gap-4 w-full">
              <div className="flex items-center justify-center gap-4 bg-black/40 backdrop-blur-md px-4 py-2.5 rounded-[2rem] border border-white/5 shadow-inner">
                <div className="flex items-center gap-1 bg-black/40 p-1 rounded-full border border-white/5">
                  {( [1, 2, 4] as const).map((d) => (<button key={d} onClick={() => setDensity(d)} className={`w-8 h-8 flex items-center justify-center rounded-full transition-all duration-150 ${density === d ? 'bg-white/10 text-white shadow-lg' : 'text-white/20 hover:text-white/50'}`}>{d === 1 ? <GridIcon1 /> : d === 2 ? <GridIcon2 /> : <GridIcon4 />}</button>))}
                </div>
                <div className="h-4 w-[1px] bg-white/10"></div>
                <button onClick={() => setHideUnowned(!hideUnowned)} className="flex items-center gap-3 group px-2"><span className={`text-[8px] font-black uppercase tracking-widest transition-colors duration-150 ${hideUnowned ? 'text-yellow-500' : 'text-white/40'}`}>OWNED</span><div className={`w-10 h-5 rounded-full relative transition-all duration-300 ${hideUnowned ? 'bg-yellow-500 shadow-[0_0_10px_rgba(234,179,8,0.3)]' : 'bg-white/10'}`}><div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform duration-300 shadow-lg ${hideUnowned ? 'translate-x-5' : 'translate-x-0'}`} /></div></button>
              </div>
              <div className="grid grid-cols-2 gap-2 p-1 bg-black/60 rounded-2xl border border-white/5 shadow-inner">{(['SLEEVES', 'BOARDS'] as const).map(tab => (<button key={tab} onClick={() => setCustomizeSubTab(tab)} className={`py-2 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-150 ${customizeSubTab === tab ? 'bg-white/10 text-white shadow-md' : 'text-white/30 hover:text-white/50'}`}>{tab}</button>))}</div>
            </div>
          }
        >Visual Assets</SectionLabel>
        {customizeSubTab === 'SLEEVES' ? (
          <div className="animate-in fade-in duration-200"><div className={`grid ${sleeveGridClass} gap-3`}>{filteredSleeves.map(s => {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-200">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between bg-black/40 backdrop-blur-md p-4 rounded-2xl border border-white/5">
           ...
           <button onClick={() => setSoundEnabled(!soundEnabled)} className={`w-12 h-6 rounded-full relative transition-colors duration-200 ${soundEnabled ? 'bg-emerald-600' : 'bg-white/10'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ${soundEnabled ? 'translate-x-7' : 'translate-x-1'}`} /></button>
        </div>
        <div className="flex items-center justify-between bg-black/40 backdrop-blur-md p-4 rounded-2xl border border-white/5">
           ...
           <button onClick={() => setPlayAnimationsEnabled(!playAnimationsEnabled)} className={`w-12 h-6 rounded-full relative transition-colors duration-200 ${playAnimationsEnabled ? 'bg-emerald-600' : 'bg-white/10'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ${playAnimationsEnabled ? 'translate-x-7' : 'translate-x-1'}`} /></button>
        </div>
        <div className="space-y-3">
          <p className="text-[9px] font-black text-white/40 uppercase tracking-widest text-center">AI Difficulty</p>
          <div className="grid grid-cols-3 gap-2 p-1 bg-black/60 backdrop-blur-md rounded-xl border border-white/5">{(['EASY', 'MEDIUM', 'HARD'] as AiDifficulty[]).map(d => (<button key={d} onClick={() => setAiDifficulty(d)} className={`py-2 rounded-lg text-[8px] font-black uppercase transition-all duration-150 ${aiDifficulty === d ? (d === 'EASY' ? 'bg-emerald-600' : d === 'MEDIUM' ? 'bg-yellow-500 text-black' : 'bg-rose-600') : 'text-white/40 hover:text-white/60'}`}>{d}</button>))}</div>
        </div>
      </div>
    </div>
  );
