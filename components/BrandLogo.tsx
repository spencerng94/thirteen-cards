...
export const BrandLogo: React.FC<BrandLogoProps> = ({ size = 'md', className = '' }) => {
  ...
  return (
    <div className={`${dimensions} ${className} relative flex items-center justify-center select-none transition-all duration-1000 ease-in-out hover:scale-[1.05] active:scale-95 group`}>
      ...
      >
        <defs>
          {/* Photorealistic Gold Gradient - Deep Amber to Pure White Specular */}
          <linearGradient id="goldMetallic" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#4b3412" /> {/* Deep Bronze Shadow */}
            <stop offset="15%" stopColor="#b8860b" /> {/* Dark Gold */}
            <stop offset="40%" stopColor="#fbf5b7" /> {/* Bright Satin Gold */}
            <stop offset="50%" stopColor="#ffffff" /> {/* High Specular Highlight */}
            <stop offset="60%" stopColor="#d4af37" /> {/* Classic Metallic Gold */}
            <stop offset="85%" stopColor="#8b6508" /> {/* Mid-tone Brass */}
            <stop offset="100%" stopColor="#4b3412" />
          </linearGradient>

          {/* Brushed Metal Texture Filter (Linear) */}
          <filter id="brushedLinearTexture" x="0%" y="0%" width="100%" height="100%">
            <feTurbulence type="fractalNoise" baseFrequency="0.9 0.01" numOctaves="2" result="noise" />
            <feColorMatrix type="saturate" values="0" in="noise" result="desaturatedNoise" />
            <feBlend in="SourceGraphic" in2="desaturatedNoise" mode="multiply" opacity="0.3" />
          </filter>

          {/* Brushed Metal Texture Filter (Radial-ish for spade) */}
          <filter id="brushedRadialTexture" x="0%" y="0%" width="100%" height="100%">
          ...
          <g filter="url(#gold3DBevel)">
            ...
            <text 
              ...
              style={{ 
                ...
                transformOrigin: '512px 720px',
              }}
            >
              THIRTEEN
            </text>

            {/* Tagline - FIRST TO ZERO WINS with Horizontal Brushed Metal Finish and Luxurious Glow */}
            <text 
              x="512" 
              y="805" 
              textAnchor="middle" 
